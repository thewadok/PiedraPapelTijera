    ...
    PiedraPapelTijeraTheme {
        ...
        MaterialTheme {
            NavHost(
                ...
                composable(Screen.History.route) { AppScaffold(nav) { HistoryScreen(gameViewModel) } }
                composable(Screen.Ranking.route) { AppScaffold(nav) { RankingScreen(gameViewModel) } }
                composable(Screen.Setting.route) { AppScaffold(nav) { SettingScreen() } }
                composable(Screen.Help.route) { AppScaffold(nav) { HelpScreen() } }
                composable(Screen.Game.route) { AppScaffold(nav) { GameScreen(nav, gameViewModel) } }
            }
        }
    }
}